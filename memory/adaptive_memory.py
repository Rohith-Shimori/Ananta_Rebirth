...
class AdaptiveMemory:
    ...
    Optimized with persistent SQLite connection and WAL mode.
    """
    ...
    def __init__(self):
        ...
        # check_same_thread=False allows us to use the connection from multiple threads
        # provided we serialize access (which we do with _lock)
        self._conn = sqlite3.connect(self.db_path, check_same_thread=False)
        self._conn.row_factory = sqlite3.Row

        # Optimization: Use Write-Ahead Logging (WAL) and reduced sync overhead
        with self._lock:
